...
import { Sliders, CheckCircle2, Save, PlusCircle, Trash2, X, RefreshCw, Layers } from 'lucide-react';
...
export const SlaConfigPage: React.FC = () => {
  ...
  const handleSave = async (ruleId: string) => {
    try {
      ...
      if (res.data.success) {
        showToast('SLA Policy thresholds updated successfully!');
  ...
  const handleCreateRule = async (e: React.FormEvent) => {
    ...
    try {
      const res = await adminApi.createSlaRule({
        ...newRuleData,
        categoryId: newRuleData.categoryId || null,
      });
      if (res.data.success) {
        showToast('New SLA Policy rule created successfully!');
  ...
  const handleDeleteRule = async (ruleId: string) => {
    if (!window.confirm('Are you sure you want to remove this SLA rule policy?')) return;
    try {
      const res = await adminApi.deleteSlaRule(ruleId);
      if (res.data.success) {
